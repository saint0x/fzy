...
- storage KV handles now follow the same owned runtime-handle law too: `KvStoreHandle` is compiler-known linear state, `storage.kv_open` / `storage.kv_close` are locked as producer/terminal operations, wrapper close/reuse/return flows are regression-covered, emitted memory/runtime-contract artifacts now track KV ownership, and the runtime shim now allocates independent open handles instead of silently aliasing one backing slot across multiple opens
- the shipped handle matrix is now explicit compiler-known law too: `.fz/handle-contracts.json` is emitted from one contract table covering linear vs non-linear handle families, HIR linear classification and memory artifacts now read from that same handle metadata, runtime import contract validation rejects producer/observer drift against the matrix, and stdlib docs now point people at the emitted handle contract artifacts instead of leaving handle semantics implicit
- provenance-root ownership is now locked at the shipped compiler surface too: alias-after-free, helper-returned pointer provenance, tuple/struct destructuring provenance, reassignment-cleared stale roots, and nested control-flow use-after-free are all regression-covered in `fz verify` and deterministic Fozzy scenarios, and provenance-root diagnostics now carry dedicated guidance instead of generic ownership help
- aggregate partial-move enforcement is now locked at the shipped compiler surface too: tuple destructuring, nested struct-field extraction, struct/enum pattern extraction, and assignment-shaped partial moves all emit one stable verifier family with explicit “move the full aggregate or borrow fields” guidance, and the pass/ fail shapes are pinned across HIR, `fz verify`, and deterministic Fozzy scenarios
