...
pub fn compile_library_incremental_with_backend(
    ...
    let snapshot_resolved = resolve_source_path_with_target(&snapshot_input, true)?;
    let pgo = configured_pgo();
    if (pgo.generate_dir.is_some() || pgo.use_profile.is_some()) && backend != "llvm" {
        bail!(
            "PGO is only supported with backend `llvm`; current backend is `{}`",
    ...
    let ((static_lib, shared_lib), incremental) = if status == "ok" {
        let (outputs, mut incremental) = emit_native_incremental_libraries(
            &lowered.fir,
            &parsed,
            &snapshot_resolved.project_root,
        ...
        remap_snapshot_incremental_report(&snapshot, &mut incremental);
        (outputs, incremental)
    } else {
        (
            (None, None),
