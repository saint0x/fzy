...
| `let_pattern_destructuring` | partial | Variant-pattern destructuring in `let` lowers natively for literal variant initializers; native `match` variant payload bindings are explicitly rejected; tuple/struct destructuring and full pattern family parity remain open. |
