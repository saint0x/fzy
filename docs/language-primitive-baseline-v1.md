...
| `pub_use_reexport_support` | implemented | `pub use ...` parses and participates in symbol resolution as stable re-export support. |
...
| `let_pattern_destructuring` | implemented | Tuple, struct, and enum variant patterns are parsed, typed, evaluated, and exercised in native backends. Native lowering uses first-class aggregate handles plus typed layout metadata for tuple/struct/enum locals, parameters, calls, returns, helper-produced values, and control-flow-produced values, with explicit runtime tag extraction for enum matches. |
...
| `closure_lambda_values` | implemented | Typed lambda expressions with lexical capture are supported in parser/HIR/FIR/evaluator and lower natively for local callable bindings (`let` + assignment). Unsupported non-local callable placements fail through native unresolved-call diagnostics. |
| `array_index_expression_family` | implemented | Array literals and index expressions are supported in parser/HIR/FIR/evaluator and lower natively in both LLVM and Cranelift through direct-memory canonical lowering (with cross-backend execute-and-compare parity coverage). |
